...
int decode_spec(context *ctx, Instruction *dec)
{
	...
	if(op0 && !op1) {
		...
		if(op0==2 && (op1&0x7f00)==0x1200) {
			/* GROUP: mortlach_zero_zt */
			op0 = (INSWORD>>4)&0x3fff;
			if(!op0)
				return decode_iclass_mortlach_zero_zt(ctx, dec);
			if(op0)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_62_mortlach_zero_zt
			UNMATCHED;
		}
		if(op0==2 && (op1&0x7f00)==0x1300) {
			...
			if(op0 && op1==2 && op2)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_65_mortlach_mov_zt
			if(op1&1)
		...
		if(op0==2 && (op1&0x4c00)==0x4000) {
			...
			if(op0==2 && !op1 && op2&1)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_147_mortlach_multi_indexed_1
			if(op0==2 && op1)
		...
		if(op0==2 && (op1&0x4c20)==0x4400) {
			...
			if(op0==2 && (op1&2)==2 && op2 && !(op3&2))
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_151_mortlach_multi_indexed_2
		...
		if(op0==2 && (op1&0x48ff)==0x482e && !(op2&2)) {
			...
			if(op0==1)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_177_mortlach_multi_sve_2d0
			if(op0==2)
		...
		if(op0==2 && (op1&0x48ff)==0x482f && !(op2&2)) {
			...
			if(op0)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_178_mortlach_multi_sve_2d1
			UNMATCHED;
		}
		...
		if(op0==2 && (op1&0x487f)==0x482c) {
			...
			if(op0==1)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_180_mortlach_multi_sve_2c0
			if(op0==2)
		...
		if(op0==2 && (op1&0x487f)==0x482d) {
			...
			if(op0)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_181_mortlach_multi_sve_2c1
			UNMATCHED;
		}
		...
		if(op0==2 && (op1&0x4838)==0x4820) {
			...
			if(op0==1 && !op1 && op2==2)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_186_mortlach_multi_sve_1
		...
		if(op0==2 && (op1&0x4838)==0x4830) {
			...
			if(op0==2 && op1==5)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_189_mortlach_multi_sve_3
			if(op0==3 && op1==5)
		...
		if(op0==2 && (op1&0x483f)==0x4839 && !(op2&0x21)) {
			...
			if(op0==1 && !op1 && op2)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_226_mortlach_multi_sve_5a
			if(op0==1 && op1)
		...
		if(op0==2 && (op1&0x483f)==0x483a && !(op2&0x21)) {
			...
			if(op0 && !op1 && op2)
				UNALLOCATED(ENC_UNKNOWN); // iclass: UNALLOCATED_228_mortlach_multi_sve_5b
			if(op0 && op1)
	...
	if(op1==2) {
		...
		if(op0==2 && !(op1&0x90) && (op2&0x3c)==0x34)
