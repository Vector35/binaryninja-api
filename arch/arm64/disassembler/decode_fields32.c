...
void decode_fields32(enum ENCODING enc, context *ctx, Instruction *instr)
{
	...
	switch(enc) {
		...
		case ENC_MADPT_Z_ZZZ_:
			// xxx|xxxx|x|opc=xx|x|Zm=xxxxx|xxxx|opc2=xx|Za=xxxxx|Zdn=xxxxx
			ctx->opc = (insword>>22)&3;
			ctx->Zm = (insword>>16)&0x1f;
		...
		case ENC_MLAPT_Z_ZZZ_:
			// xxx|xxxx|x|opc=xx|x|Zm=xxxxx|xxxx|opc2=xx|Zn=xxxxx|Zda=xxxxx
			ctx->opc = (insword>>22)&3;
			ctx->Zm = (insword>>16)&0x1f;
			ctx->opc2 = (insword>>10)&3;
			ctx->Zn = (insword>>5)&0x1f;
		...
		case ENC_UQRSHRN_Z_MZ2_:
			// xxx|xxxx|x|x|tszh=x|x|tszl=x|imm4=xxxx|x|x|op=x|U=x|R=x|x|Zn=xxxx|x|Zd=xxxxx
			ctx->tszh = (insword>>22)&1;
			ctx->tszl = (insword>>20)&1;
			ctx->imm4 = (insword>>16)&15;
			ctx->op = (insword>>13)&1;
			ctx->U = (insword>>12)&1;
			ctx->R = (insword>>11)&1;
