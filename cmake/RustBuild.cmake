...
#   Crates register with build groups. Cargo is invoked once per group to avoid lock
#   contention. Each crate specifies a GROUP (e.g. "plugins", "internal").
#
...
if(NOT DEFINED BN_API_SOURCE_DIR)
    set(BN_API_SOURCE_DIR ${_BN_API_PATH})
endif()
...
# Get the cargo profile name as spelled in CARGO_PROFILE_<NAME>_* environment variables
function(_bn_get_cargo_profile_env_name OUT_VAR)
    _bn_get_cargo_profile(_unused PROFILE_DIR)
    if(PROFILE_DIR STREQUAL "debug")
        set(PROFILE_NAME dev)
    else()
        set(PROFILE_NAME ${PROFILE_DIR})
    endif()
    string(TOUPPER ${PROFILE_NAME} PROFILE_NAME)
    string(REPLACE "-" "_" PROFILE_NAME ${PROFILE_NAME})
    set(${OUT_VAR} ${PROFILE_NAME} PARENT_SCOPE)
endfunction()
...
function(_bn_gather_plugin_sources OUT_VAR)
    ...
    if(TARGET binaryninjaapi)
        ...
        # binaryninjacore-sys reads both headers to generate its bindings
        list(APPEND SOURCES
            ${BN_API_SOURCE_DIR}/binaryninjacore.h
            ${BN_API_SOURCE_DIR}/ui/uitypes.h)
    endif()
...
function(_bn_create_build_command)
    cmake_parse_arguments(ARG
        ""
        "COPY_TO;OUTPUT_FILE_NAME;OUTPUT_TYPE;TARGET_NAME;WORKSPACE;MANIFEST_PATH;TARGET_DIR;RPATH_KIND"
        "CARGO_OPTS;PACKAGE_ARGS;DEPENDS;OUTPUT_FILES;COPY_FROM_FILES;BYPRODUCTS"
        ${ARGN})

    if(NOT ARG_RPATH_KIND)
        set(ARG_RPATH_KIND plugin)
    endif()

    set(ALL_COMMANDS "")
    set(CMD_OUTPUT "")
    set(CMD_DEPENDS ${ARG_DEPENDS})
    set(CMD_BYPRODUCTS ${ARG_BYPRODUCTS})
    ...
    if(ARG_CARGO_OPTS)
        ...
        # rustc links bundled native static libraries from temporary copies of their rlibs
        # and deletes them afterwards, so a packaging-time dsymutil cannot find those objects.
        # Packed split-debuginfo makes rustc produce the dSYM itself at link time.
        if(APPLE AND ARG_OUTPUT_TYPE STREQUAL "SHARED")
            _bn_get_cargo_profile_env_name(PROFILE_ENV_NAME)
            list(APPEND CARGO_ENV_VARS CARGO_PROFILE_${PROFILE_ENV_NAME}_SPLIT_DEBUGINFO=packed)
        endif()
    ...
    if(ARG_COPY_TO)
        ...
        if(APPLE AND UNIVERSAL)
            ...
            # Stage the dSYM next to the library, merging the per-architecture DWARF slices
            if(ARG_OUTPUT_TYPE STREQUAL "SHARED")
                set(_DSYM_DWARF Contents/Resources/DWARF/${ARG_OUTPUT_FILE_NAME})
                list(APPEND ALL_COMMANDS
            ...
            # Stage the dSYM next to the library
            if(APPLE AND ARG_OUTPUT_TYPE STREQUAL "SHARED")
                list(APPEND ALL_COMMANDS
                    COMMAND ${CMAKE_COMMAND} -E rm -rf ${ARG_COPY_TO}.dSYM
            ...
            if(UNIX AND NOT APPLE AND ARG_OUTPUT_TYPE STREQUAL "SHARED")
                list(APPEND ALL_COMMANDS
                    COMMAND ${CMAKE_COMMAND} -DRPATH_KIND=${ARG_RPATH_KIND} -DBINARY_PATH=${ARG_COPY_TO}
                        -P ${_BN_RUST_CMAKE_DIR}/SetElfRpath.cmake)
            endif()
    ...
    if(CMD_BYPRODUCTS)
        set(BYPRODUCTS_ARG BYPRODUCTS ${CMD_BYPRODUCTS})
    else()
        set(BYPRODUCTS_ARG "")
    endif()
...
function(bn_add_rust_static_umbrella UMBRELLA_TARGET)
    cmake_parse_arguments(ARG "" "OUTPUT_DIR;LOCKFILE" "GROUPS;DEPENDS" ${ARGN})
    ...
    # The umbrella is a workspace root of its own with no committed lock. Seeding it from
    # LOCKFILE keeps those pins. Cargo resolves only what the remaining crates add.
    if(ARG_LOCKFILE AND NOT EXISTS ${UMBRELLA_GEN_DIR}/${UMBRELLA_TARGET}/Cargo.lock)
        configure_file(${ARG_LOCKFILE} ${UMBRELLA_GEN_DIR}/${UMBRELLA_TARGET}/Cargo.lock COPYONLY)
    endif()
    ...
    # GNU ld requires libraries to appear in dependency order, so these must follow the umbrella.
    if(ALL_LINK_DEPS)
        target_link_libraries(${UMBRELLA_TARGET}_deps INTERFACE ${ALL_LINK_DEPS})
        set_property(TARGET ${UMBRELLA_TARGET} APPEND PROPERTY
            INTERFACE_LINK_LIBRARIES ${UMBRELLA_TARGET}_deps)
    endif()
