...
|debugger|Apply the target memory map as segments|When enabled and the debug adapter reports a memory map, the debugger mirrors it into the binary view as one bounded segment per mapped region, which lets Find work during debugging (it only scans mapped memory). When disabled, the debugger falls back to a single overlay spanning the whole address space (the previous behavior). Adapters that do not report a memory map always use the overlay.|`boolean`|`True`|[`SettingsUserScope`]|<a id='debugger.useMemoryMapSegments'>debugger.useMemoryMapSegments</a>|
...
|platform|Automatically Load Platform Import Libraries|Automatically load import libraries during platform initialization.|`boolean`|`True`|[`SettingsUserScope`]|<a id='platform.autoLoadImportLibs'>platform.autoLoadImportLibs</a>|
...
|ui.experimental|Importlib Explorer|Enables Import Library Explorer tools. Requires a restart and creates a new sidebar icon that can be enabled.|`boolean`|`False`|[`SettingsUserScope`]|<a id='ui.experimental.importlibExplorer'>ui.experimental.importlibExplorer</a>|
