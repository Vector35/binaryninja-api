...
|rendering.format|Allow Wide Lines|If the view is wider than the default line length, allow lines to expand to fit the view instead of wrapping.|`boolean`|`False`|[`SettingsUserScope`]|<a id='rendering.format.allowWideLines'>rendering.format.allowWideLines</a>|
