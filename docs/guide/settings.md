...
|analysis|Type Convergence Detection|Detect and mitigate non-converging type inference patterns, including recursive function pointer growth, pointer depth growth, and type oscillation.|`boolean`|`True`|[`SettingsProjectScope`, `SettingsResourceScope`, `SettingsUserScope`]|<a id='analysis.typeConvergenceDetection'>analysis.typeConvergenceDetection</a>|
...
|analysis.jni|JNI Type Application|Detects functions matching JNI naming conventions (Java_*, JNI_*) and applies appropriate JNI parameter types such as JNIEnv*, jobject, and JavaVM*.|`boolean`|`True`|[`SettingsProjectScope`, `SettingsResourceScope`, `SettingsUserScope`]|<a id='analysis.jni.applyTypes'>analysis.jni.applyTypes</a>|
|analysis.limits|Advanced Analysis Cache Ceiling Factor|Multiplier used to decide when to collect during active analysis. Collection is triggered when resident advanced analysis data exceeds the cache retention size multiplied by this factor. Higher values reduce sweep frequency but allow higher peak memory use.|`number`|`2`|[`SettingsProjectScope`, `SettingsResourceScope`, `SettingsUserScope`]|<a id='analysis.limits.cacheCeilingFactor'>analysis.limits.cacheCeilingFactor</a>|
...
|analysis.limits|Max Function Re-dispatch Count|Maximum number of times a function is re-analyzed within a single advanced analysis iteration before its remaining analysis is deferred to the next iteration. A value of 0 limits each function to one update per iteration.|`number`|`3`|[`SettingsProjectScope`, `SettingsResourceScope`, `SettingsUserScope`]|<a id='analysis.limits.maxFunctionRedispatchCount'>analysis.limits.maxFunctionRedispatchCount</a>|
...
|collaboration|Active Server|URL of the server to automatically connect to on launch. (Requires the collaboration add-on)|`string`| |[`SettingsProjectScope`, `SettingsUserScope`]|<a id='collaboration.activeRemote'>collaboration.activeRemote</a>|
|collaboration|Advanced Conflict Resolution|Show extra information when resolving merge conflicts (Requires the collaboration add-on)|`boolean`|`False`|[`SettingsProjectScope`, `SettingsUserScope`]|<a id='collaboration.advancedMerge'>collaboration.advancedMerge</a>|
|collaboration|Auto Connect|Automatically reconnect to last collaboration server if you were connected when Binary Ninja was closed. (Requires the collaboration add-on)|`boolean`|`True`|[`SettingsProjectScope`, `SettingsResourceScope`, `SettingsUserScope`]|<a id='collaboration.autoConnectOnLaunch'>collaboration.autoConnectOnLaunch</a>|
|collaboration|Automatically Download Project File Dependencies|Try to automatically download file dependencies when opening or downloading a remote project file. (Requires the collaboration add-on)|`boolean`|`True`|[`SettingsProjectScope`, `SettingsUserScope`]|<a id='collaboration.autoDownloadFileDependencies'>collaboration.autoDownloadFileDependencies</a>|
|collaboration|Poll Interval|Time in seconds between polls for new snapshots. 0 to disable. (Requires the collaboration add-on)|`number`|`30`|[`SettingsUserScope`]|<a id='collaboration.autoPollInterval'>collaboration.autoPollInterval</a>|
|collaboration|Collaboration Project Directory|Default path for files downloaded from a remote (Requires the collaboration add-on)|`string`|`/Users/jwiens/vector35/empty_profile/remote`|[`SettingsProjectScope`, `SettingsUserScope`]|<a id='collaboration.directory'>collaboration.directory</a>|
|collaboration|Synchronize Local Project Deletions on Open|When opening a remote project, if you have deleted files/folders locally, delete them on the remote as well. (Requires the collaboration add-on)|`boolean`|`False`|[`SettingsUserScope`]|<a id='collaboration.localProjectDeletionSynchronization'>collaboration.localProjectDeletionSynchronization</a>|
|collaboration|Maximum Conflict Diff Size|Any conflicts larger than this will not be displayed with a diff, so that the UI doesn't hang. (Requires the collaboration add-on)|`number`|`200000`|[`SettingsProjectScope`, `SettingsUserScope`]|<a id='collaboration.maxConflictDiff'>collaboration.maxConflictDiff</a>|
|collaboration|Collaboration Servers|Collaboration servers to use (Requires the collaboration add-on)|`string`|`{}`|[`SettingsProjectScope`, `SettingsUserScope`]|<a id='collaboration.servers'>collaboration.servers</a>|
|collaboration|Sync On Save|Sync to connected remote on save of collaborative database (Requires the collaboration add-on)|`boolean`|`False`|[`SettingsUserScope`]|<a id='collaboration.syncOnSave'>collaboration.syncOnSave</a>|
...
|corePlugins|JNI Plugin|Enable the built-in JNI plugin, which applies JNI types to functions matching JNI naming conventions (Java_*, JNI_*).|`boolean`|`True`|[`SettingsUserScope`]|<a id='corePlugins.jni'>corePlugins.jni</a>|
|corePlugins|Metric Similarity Plugin|Enable the built-in metric similarity plugin.|`boolean`|`True`|[`SettingsUserScope`]|<a id='corePlugins.metricSimilarity'>corePlugins.metricSimilarity</a>|
...
|corePlugins.workflows|Swift|Enable the built-in Swift plugin.|`boolean`|`True`|[`SettingsUserScope`]|<a id='corePlugins.workflows.swift'>corePlugins.workflows.swift</a>|
|crashReporting|Automatically Send Crash Reports|Crash reports contain operating system version, call stack contents, and loaded native libraries. No binary content or identifying information is intentionally included, however application and plug-in paths may contain your system username.|`boolean`|`False`|[`SettingsUserScope`]|<a id='crashReporting.enabled'>crashReporting.enabled</a>|
...
|debugger|Auto-Rebase on Module Load|When enabled, automatically rebase the input binary view to match the remote base address when the module is loaded. Disable this to keep the original base address and use the 'Rebase to Remote Base' action in the Debugger menu to manually trigger rebasing.|`boolean`|`True`|[`SettingsUserScope`]|<a id='debugger.autoRebase'>debugger.autoRebase</a>|
|debugger|Case Insensitive Module Name Matching|When enabled, module name comparisons are case-insensitive. This is useful when debug adapters report module names with different casing than the actual binary file names.|`boolean`|`True`|[`SettingsUserScope`]|<a id='debugger.caseInsensitiveModuleName'>debugger.caseInsensitiveModuleName</a>|
...
|debugger|Apply the target memory map as segments|When enabled and the debug adapter reports a memory map, the debugger mirrors it into the binary view as one bounded segment per mapped region, which lets Find work during debugging (it only scans mapped memory). This is off by default because querying the memory map on every stop is slow with adapters that report one region per request, e.g., LLDB. When disabled, the debugger uses a single overlay spanning the whole address space. Adapters that do not report a memory map always use the overlay.|`boolean`|`False`|[`SettingsUserScope`]|<a id='debugger.useMemoryMapSegments'>debugger.useMemoryMapSegments</a>|
|enterprise|Automatically Checkout License|If enabled, Binary Ninja will automatically check out a license with chosen Default License Checkout Duration on startup. (Requires a floating license)|`boolean`|`False`|[`SettingsUserScope`]|<a id='enterprise.autoCheckout'>enterprise.autoCheckout</a>|
|enterprise|Automatically Refresh Floating License|If enabled, Binary Ninja will automatically refresh floating licenses as they near expiration. (Requires a floating license)|`boolean`|`True`|[`SettingsUserScope`]|<a id='enterprise.autoRefreshFloatingLicense'>enterprise.autoRefreshFloatingLicense</a>|
|enterprise|Default License Checkout Duration|Specify the default selection for license checkout length when opening Binary Ninja. (Requires a floating license)|`string`|`Until Quit`|[`SettingsUserScope`]|<a id='enterprise.defaultCheckoutDuration'>enterprise.defaultCheckoutDuration</a>|
|enterprise|Secrets Provider|Handles secret storage, which is used for floating licenses and collaboration.|`string`|`AESFileSecretsProvider`|[`SettingsUserScope`]|<a id='enterprise.secretsProvider'>enterprise.secretsProvider</a>|
| | | |`enum`|`SystemSecretsProvider`| | |
| | | |`enum`|`AESFileSecretsProvider`| | |
|enterprise.server|Enterprise Server Additional Headers|Additional headers to send with requests to Enterprise Servers. Must be a valid JSON string of the form {"https://example.com:443": {"x-foo": "bar"}} (Requires a floating license or the collaboration add-on)|`string`|`{}`|[`SettingsUserScope`]|<a id='enterprise.server.additionalHeaders'>enterprise.server.additionalHeaders</a>|
|enterprise.server|Enterprise Server URL|Base url for connected Enterprise Server. (Requires a floating license or the collaboration add-on)|`string`| |[`SettingsUserScope`]|<a id='enterprise.server.url'>enterprise.server.url</a>|
|extensionManager|Community Plugin Repository|Whether the community plugin repository is enabled|`boolean`|`True`|[`SettingsUserScope`]|<a id='extensionManager.communityRepo'>extensionManager.communityRepo</a>|
|extensionManager|Debug Extension Manager|Internal use only. May prevent your ability to install official and community plugins.|`boolean`|`False`|[`SettingsUserScope`]|<a id='extensionManager.debug'>extensionManager.debug</a>|
|extensionManager|Official Plugin Repository|Whether the official plugin repository is enabled|`boolean`|`True`|[`SettingsUserScope`]|<a id='extensionManager.officialRepo'>extensionManager.officialRepo</a>|
|extensionManager|Unofficial 3rd Party Plugin Repository URLs|Specify URLs of third party plugin repositories|`array`|`[]`|[`SettingsUserScope`]|<a id='extensionManager.unofficialUrls'>extensionManager.unofficialUrls</a>|
|extensionManager|Use Short URL|When enabled, will use a shortened URL for package installation which contributes to total install counts.|`boolean`|`True`|[`SettingsUserScope`]|<a id='extensionManager.useShortUrl'>extensionManager.useShortUrl</a>|
...
|loader.macho|Mach-O Rebase/Bind Table Entry Count Limit Multiplier|Multiplier applied to the maximum number of rebase/bind entries permitted per table, which is derived from the size of the Mach-O slice divided by its pointer size|`number`|`1.0`|[`SettingsProjectScope`, `SettingsResourceScope`, `SettingsUserScope`]|<a id='loader.macho.maxRebaseBindEntriesMultiplier'>loader.macho.maxRebaseBindEntriesMultiplier</a>|
...
|network|Enable Collaboration Server|Allow Binary Ninja to connect to collaboration servers to list Projects and Sync changes. (Requires the collaboration add-on)|`boolean`|`True`|[`SettingsUserScope`]|<a id='network.enableCollaborationServer'>network.enableCollaborationServer</a>|
...
|network|Enable Extension Manager Networking|Allow Binary Ninja to connect to GitHub and to check for new plugins and plugin updates.|`boolean`|`True`|[`SettingsUserScope`]|<a id='network.enableExtensionManager'>network.enableExtensionManager</a>|
...
|rendering.format|Allow Wide Lines (Experimental)|If the view is wider than the default line length, allow lines to expand to fit the view instead of wrapping. This may cause navigation and token selection issues.|`boolean`|`False`|[`SettingsUserScope`]|<a id='rendering.format.allowWideLines'>rendering.format.allowWideLines</a>|
...
|ui|Scrollbar Width|Width of scrollbars in logical pixels. Set to 0 to use the current theme's default.|`number`|`0`|[`SettingsUserScope`]|<a id='ui.scrollbarWidth'>ui.scrollbarWidth</a>|
|ui.commandPalette|Limit Per Type When Filtering|Maximum items shown per type when searching|`number`|`100`|[`SettingsUserScope`]|<a id='ui.commandPalette.filterLimitPerType'>ui.commandPalette.filterLimitPerType</a>|
|ui.commandPalette|Recent Command Limit|Maximum recent items displayed in command palette.|`number`|`5`|[`SettingsUserScope`]|<a id='ui.commandPalette.recentLimit'>ui.commandPalette.recentLimit</a>|
|ui.commandPalette|Recent Command Limit|Maximum recent items remembered for command palette. Count shown is controlled by ui.commandPalette.recentLimit.|`number`|`100`|[`SettingsUserScope`]|<a id='ui.commandPalette.storageCount'>ui.commandPalette.storageCount</a>|
...
|ui.mcp|MCP Server Enabled|Start the local Model Context Protocol server for exposing Binary Ninja file manager functionality.|`boolean`|`False`|[`SettingsUserScope`]|<a id='ui.mcp.enabled'>ui.mcp.enabled</a>|
|ui.mcp|MCP Server Endpoint|HTTP endpoint path used by the Binary Ninja MCP server.|`string`|`/mcp`|[`SettingsUserScope`]|<a id='ui.mcp.endpoint'>ui.mcp.endpoint</a>|
|ui.mcp|MCP Server Port|Port used by the Binary Ninja MCP HTTP server. Use 0 for an OS-assigned port.|`number`|`24642`|[`SettingsUserScope`]|<a id='ui.mcp.port'>ui.mcp.port</a>|
|ui.mcp|MCP Server Bearer Token|Bearer token required by the Binary Ninja MCP HTTP server. Leave blank to disable HTTP Authorization.|`string`| |[`SettingsUserScope`]|<a id='ui.mcp.token'>ui.mcp.token</a>|
...
|updates|Use Enterprise Server For Updates|Use the enterprise server to check for updates. (Requires a floating license or the collaboration add-on)|`boolean`|`False`|[`SettingsUserScope`]|<a id='updates.useEnterpriseServer'>updates.useEnterpriseServer</a>|
