	...
	optional<bool> signedHint)
{
	...
	case HLIL_ZX:
		[&]() {
			...
			tokens.AppendCloseParen();
			GetExprTextInternal(srcExpr, tokens, settings, asFullAst, precedence, false, false);
			tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
		}();
	...
	case HLIL_SX:
		[&]() {
			...
			tokens.AppendCloseParen();
			GetExprTextInternal(srcExpr, tokens, settings, asFullAst, precedence, false, true);
			tokens.AppendCloseParen();
			if (statement)
				tokens.AppendSemicolon();
		}();
