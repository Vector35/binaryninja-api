...
private:
	...
	std::vector<BinaryNinja::Variable> activeSyncVars;

	bool TryEmitAutoreleasepoolBlock(const BinaryNinja::Function& function,
		std::span<const BinaryNinja::HighLevelILInstruction> statements, size_t index,
		BinaryNinja::HighLevelILTokenEmitter& tokens, BinaryNinja::DisassemblySettings* settings, size_t& out_consumed);
	bool TryEmitSynchronizedBlock(const BinaryNinja::Function& function,
		std::span<const BinaryNinja::HighLevelILInstruction> statements, size_t index,
		BinaryNinja::HighLevelILTokenEmitter& tokens, BinaryNinja::DisassemblySettings* settings, size_t& out_consumed);
