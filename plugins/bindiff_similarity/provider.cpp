	...
	SimilarityProviderResults& results, SimilaritySessionCompletion& completion)
{
	...
	std::vector<PendingMatch> pendingMatches;
	{
		...
		for (const auto& fixedPoint : fixedPoints)
		{
			...
			const double confidence = fixedPoint.GetConfidence();
			...
			const double normalizedConfidence = std::isfinite(confidence) ? std::clamp(confidence, 0.0, 1.0) : 0.0;
			const auto similarityScore = static_cast<uint8_t>(std::lround(normalizedSimilarity * 255));
			const auto confidenceScore = static_cast<uint8_t>(std::lround(normalizedConfidence * 255));
			...
			pendingMatches.push_back(
				{{primaryRef, secondaryRef}, similarityScore, confidenceScore, primaryName, secondaryName});
		}
	...
	for (const auto& match : pendingMatches)
	{
		const SimilarityResultId primaryResult =
			results.AddResult(match.key.primary, match.key.secondary, match.similarity, match.confidence);
		const SimilarityResultId secondaryResult =
			results.AddResult(match.key.secondary, match.key.primary, match.similarity, match.confidence);
