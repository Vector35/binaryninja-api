...
class GoogleSimilarityProvider : public BinaryNinja::SimilarityProvider
{
	...
	struct RenderData
	{
		struct BlockComparison
		{
			...
			size_t otherInstructionCount;
			std::optional<size_t> matchedInstructionCount;
		};

		struct Source
		{
			BNSimilarityAnnotationType unmatchedType;
			std::vector<BlockComparison> blocks;
		};
	...
	std::unordered_map<MatchKey, RenderData, MatchKeyHash> m_renderData;
