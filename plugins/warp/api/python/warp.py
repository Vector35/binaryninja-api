...
class WarpFunctionComment:
    ...
    def __repr__(self):
        return f"<FunctionComment '{self.text}': 0x{self.offset:x}>"
...
class WarpConstraint:
    def __init__(self, handle: warpcore.BNWARPConstraint):
        self.handle = handle

    def __del__(self):
        if self.handle is not None:
            warpcore.BNWARPFreeConstraintReference(self.handle)

    def __repr__(self):
        if self.offset is None:
            return f"<WarpConstraint '{self.guid}'>"
        return f"<WarpConstraint '{self.guid}': 0x{self.offset:x}>"

    @property
    def guid(self) -> ConstraintGUID:
        return ConstraintGUID(warpcore.BNWARPConstraintGetGUID(self.handle))

    @property
    def offset(self) -> Optional[int]:
        offset = warpcore.BNWARPConstraintGetOffset(self.handle)
        if offset == -1:  # Assuming -1 indicates no offset
            return None
        return offset
...
class WarpFunction:
    ...
    def constraints(self) -> List[WarpConstraint]:
        ...
        for i in range(count.value):
            result.append(WarpConstraint(warpcore.BNWARPNewConstraintReference(constraints[i])))
