    ...
    : QDialog(parent), m_fetchProcessor(std::move(fetcher)), m_bv(std::move(bv))
{
    ...
    // shift existing container names after "All"
    // TODO: Make this strip the leading text until it is a reasonable size.
    ...
    srcBtnRow->addWidget(m_addSourceBtn);
    srcBtnRow->addWidget(m_removeSourceBtn);
    auto srcCol = new QVBoxLayout();
    srcCol->addWidget(m_sourcesList);
    srcCol->addLayout(srcBtnRow);
    auto srcWrapper = new QWidget(this);
    srcWrapper->setLayout(srcCol);
    ...
    form->addRow(new QLabel("Source GUIDs: "), srcWrapper);
    ...
    // Wire buttons
    connect(m_addSourceBtn, &QPushButton::clicked, this, &WarpFetchDialog::onAddSource);
    connect(m_removeSourceBtn, &QPushButton::clicked, this, &WarpFetchDialog::onRemoveSource);
...
void WarpFetchDialog::onAddSource()
{
    bool ok = false;
    const auto text = QInputDialog::getText(this, "Add Source GUID", "GUID:", QLineEdit::Normal, {}, &ok);
    if (ok)
        AddListItem(m_sourcesList, text);
}

void WarpFetchDialog::onRemoveSource()
{
    for (auto *item: m_sourcesList->selectedItems())
        delete item;
}
...
std::vector<std::string> WarpFetchDialog::collectSources() const
{
    std::vector<std::string> out;
    out.reserve(m_sourcesList->count());
    for (int i = 0; i < m_sourcesList->count(); ++i)
        out.emplace_back(m_sourcesList->item(i)->text().trimmed().toStdString());
    return out;
}
...
void WarpFetchDialog::onAccept()
{
    ...
    runBatchedFetch(containerIndex, sources, tags, batch, rerun);

    if (rerun)
    {
        WorkerInteractiveEnqueue([this]() {
            Warp::RunMatcher(*m_bv);
        });
    }

    // TODO: I think we want to submit the dialog, then run the fetched batcher and matcher, so we dont freeze the UI.
    accept();
}
...
void RegisterWarpFetchFunctionsCommand(std::shared_ptr<WarpFetcher> fetcher)
{
    ...
    UIActionHandler::globalActions()->bindAction(
        actionName,
        UIAction(
            [fetcher](const UIActionContext &context) {
                if (const BinaryViewRef bv = context.binaryView; bv)
                {
                    WarpFetchDialog dlg(bv, fetcher, nullptr);
                    dlg.exec();
                }
            },
