#!/usr/bin/env python
# demonstrate creating an import library
...
importlib = binaryninja.importlibrary.ImportLibrary.new(arch, 'libtest.so.1')
importlib.add_platform(binaryninja.Platform['mac-x86_64'])
importlib.add_alternate_name('libtest.so')
...
importlib.add_named_type('MyVoidType', Type.void())

# example: BoolTypeClass
importlib.add_named_type('MyBoolType', Type.bool())

# example: IntegerTypeClass
importlib.add_named_type('MyCharType', Type.char())
importlib.add_named_type('MyIntType', Type.int(4, True))
importlib.add_named_type('MyUnsignedIntType', Type.int(4, False))

# example: FloatTypeClass
importlib.add_named_type('MyFloatType', Type.float(4))

# example: PointerTypeClass
# char *
importlib.add_named_type('MyPointerType', Type.pointer(arch, Type.char()))

# example of typedef to primitive type
# typedef int MyTypedefType;
importlib.add_named_type('MyTypedefType', Type.int(4))
...
with StructureBuilder.builder(importlib, 'Rectangle') as struct_type:
...
with StructureBuilder.builder(importlib, 'Rectangle2') as struct_type:
	struct_type.append(Type.int(4), 'width')
	struct_type.append(Type.int(4), 'height')
...
importlib.add_named_type('MyEnumerationType', enum_type)

# example: ArrayTypeClass
#
# unsigned char[256]
importlib.add_named_type('MyArrayType', Type.array(Type.int(1), 256))
...
importlib.add_named_type('MyFunctionType', ftype)
...
ftype = Type.function(ret, params)
importlib.add_named_object('_MySuperComputation', ftype)

# finalize
importlib.finalize()
print('writing test.bntl')
importlib.write_to_file('test.bntl')
