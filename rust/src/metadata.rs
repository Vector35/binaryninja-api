...
impl Metadata {
    ...
    pub fn get(&self, key: impl AsCStr) -> Result<Option<Ref<Metadata>>, ()> {
        if self.get_type() != MetadataType::KeyValueDataType {
            return Err(());
        }
        let ptr = unsafe { BNMetadataGetForKey(self.handle, key.as_cstr().as_ptr()) };
        if ptr.is_null() {
            return Ok(None);
        }
        Ok(Some(unsafe { Self::ref_from_raw(ptr) }))
    ...
    pub fn insert(&self, key: impl AsCStr, value: &Metadata) -> Result<(), ()> {
        if self.get_type() != MetadataType::KeyValueDataType {
            return Err(());
        }
    ...
    pub fn remove_key(&self, key: impl AsCStr) -> Result<(), ()> {
        if self.get_type() != MetadataType::KeyValueDataType {
            return Err(());
        }
...
impl<S: AsCStr> From<HashMap<S, Ref<Metadata>>> for Ref<Metadata> {
    fn from(value: HashMap<S, Ref<Metadata>>) -> Self {
        let keys = value.keys().map(|k| k.as_cstr()).collect::<Vec<_>>();
        let mut keys = keys.iter().map(|k| k.as_ptr()).collect::<Vec<_>>();
        let mut values = value.values().map(|v| v.handle).collect::<Vec<_>>();
    ...
    for<'a> &'a T: Into<Ref<Metadata>>,
{
    fn from(value: &[(S, T)]) -> Self {
        let keys = value.iter().map(|(k, _)| k.as_cstr()).collect::<Vec<_>>();
        let mut keys = keys.iter().map(|k| k.as_ptr()).collect::<Vec<_>>();
        let mut values = value
            .iter()
            .map(|(_, v)| v.into().handle)
            .collect::<Vec<_>>();
...
impl<S: AsCStr> From<Vec<S>> for Ref<Metadata> {
    fn from(value: Vec<S>) -> Self {
        let value = value.iter().map(|s| s.as_cstr()).collect::<Vec<_>>();
        let mut pointers = value.iter().map(|s| s.as_ptr()).collect::<Vec<_>>();
        unsafe {
