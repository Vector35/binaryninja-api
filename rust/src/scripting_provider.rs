...
pub trait CustomScriptingInstance {
    ...
    ///
    /// Make sure to override this _and_ [`CustomScriptingInstance::complete_arguments`] to enable argument completion.
    ...
    ///
    /// Make sure to override this _and_ [`CustomScriptingInstance::can_complete_arguments`] to enable argument completion.
    ///
    ...
    ///
    /// NOTE: Due to usage being potentially identical to when the instance is dropped, this **may be
...
impl ScriptingInstance {
    ...
    pub fn from_custom<C: CustomScriptingInstance>(
        ...
        let mut callbacks = BNScriptingInstanceCallbacks {
            ...
            stop: Some(cb_stop::<C>),
            canCompleteArguments: Some(cb_can_complete_arguments::<C>),
            completeArguments: Some(cb_complete_arguments::<C>),
        };
    ...
    pub fn can_complete_arguments(&self, text: &str) -> bool {
        let text = text.to_cstr();
        unsafe { BNScriptingInstanceCanCompleteArguments(self.handle, text.as_ptr()) }
    }
    ...
    pub fn complete_arguments(&self, text: &str) -> (String, u64) {
        ...
        let mut pos = 0;
        let result =
            unsafe { BNScriptingInstanceCompleteArguments(self.handle, text.as_ptr(), &mut pos) };
        if result.is_null() {
            ("".to_string(), 0)
        } else {
...
unsafe extern "C" fn cb_can_complete_arguments<S: CustomScriptingInstance>(
    ctxt: *mut c_void,
    text: *const c_char,
) -> bool {
    let ctxt = &mut *(ctxt as *mut CustomScriptingInstanceContext<S>);
    let text = CStr::from_ptr(text);
    ctxt.instance
        .can_complete_arguments(&text.to_string_lossy())
}

unsafe extern "C" fn cb_complete_arguments<S: CustomScriptingInstance>(
    ctxt: *mut c_void,
    text: *const c_char,
    arg_pos: *mut u64,
) -> *mut c_char {
    let ctxt = &mut *(ctxt as *mut CustomScriptingInstanceContext<S>);
    let text = CStr::from_ptr(text);
    let (result, pos) = ctxt.instance.complete_arguments(&text.to_string_lossy());
    *arg_pos = pos;
    BnString::into_raw(BnString::new(result))
}
