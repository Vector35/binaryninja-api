...
pub fn register_transform<C: CustomTransform>(custom: C) -> (&'static mut C, Transform) {
    ...
    let mut callbacks = BNCustomTransform {
        ...
        decodeWithContext: Some(cb_decode_with_context::<C>),
        canDecode: Some(cb_can_decode::<C>),
    };
    let result = unsafe {
...
pub trait CustomTransform {
    ...
    // TODO: Must support context capability
...
unsafe extern "C" fn cb_decode_with_context<C: CustomTransform>(
    ctxt: *mut c_void,
...
unsafe extern "C" fn cb_can_decode<C: CustomTransform>(
    ctxt: *mut c_void,
