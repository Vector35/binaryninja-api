...
// impl<T: RefCountable> From<Ref<T>> for Conf<&T> {
//     fn from(contents: Ref<T>) -> Self {
//         Self::new(contents.as_ref(), max_confidence())
//     }
// }

// impl<T: Clone> From<&Conf<T>> for Conf<T> {
//     fn from(contents: &Conf<T>) -> Self {
//         Self::new(contents.contents.clone(), contents.confidence)
//     }
// }
...
impl From<BNCallingConventionWithConfidence> for Conf<Ref<CallingConvention>> {
    fn from(cc_with_confidence: BNCallingConventionWithConfidence) -> Self {
        Self::new(
            unsafe { CallingConvention::ref_from_raw(cc_with_confidence.convention) },
...
impl Into<BNCallingConventionWithConfidence> for Conf<&CallingConvention> {
...
impl TypeBuilder {
    ...
    pub fn calling_convention(&self) -> Result<Conf<Ref<CallingConvention>>> {
    ...
    pub fn get_enumeration(&self) -> Result<Enumeration> {
        ...
        if result.is_null() {
            ...
            Ok(Enumeration::from_raw(result))
        }
    }
    ...
    pub fn structure(structure_type: &mut Structure) -> Self {
...
impl Type {
    ...
    pub fn structure<'a, T: Into<&'a Structure>>(structure: T) -> Ref<Self> {
        unsafe { Self::ref_from_raw(BNCreateStructureType(structure.into().handle)) }
    }
    ...
    pub fn function<'a, S: BnStrCompatible + Copy, T: Into<Conf<&'a Type>>>(
        ...
        calling_convention: Option<Conf<&CallingConvention>>,
        ...
        let calling_convention_ref =
            Conf::<&CallingConvention>::new(unsafe { mem::zeroed() }, min_confidence());
...
impl EnumerationMember {
    ...
    pub unsafe fn from_raw(handle: *mut BNEnumerationMember) -> Self {
        Self {
...
impl Enumeration {
    pub fn new(builder: &EnumerationBuilder) -> Ref<Self> {
        unsafe {
            ...
            Ref::new(Enumeration { handle })
        }
    }

    pub(crate) fn from_raw(handle: *mut BNEnumeration) -> Self {
        debug_assert!(!handle.is_null());
        Enumeration { handle }
    }
...
impl Structure {
    ...
    pub(crate) unsafe fn from_raw(handle: *mut BNStructure) -> Self {
        debug_assert!(!handle.is_null());
        Structure { handle }
    }
