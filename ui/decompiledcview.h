...
class BINARYNINJAUIAPI DecompiledCView: public TokenizedTextView
{
	Q_OBJECT
...
protected:
	...
	{
		DefaultScopeStyle,
		BracesOnSameLineStyle,
		BracesOnNewLineStyle
	};

	// This is stolen from core/disassembly.h
	...
	class DecompiledCTokenEmitter
	{
		BinaryNinja::DisassemblyTextRenderer* m_renderer;
		std::vector<BinaryNinja::DisassemblyTextLine> m_lines, m_pendingLines;
		size_t m_indent = 0;
		size_t m_activeLineIndent = 0;
		BinaryNinja::ILSourceLocation m_location;
		size_t m_exprIndex = BN_INVALID_EXPR;
		size_t m_instrIndex = 0;
		size_t m_pendingInstrIndex = 0;
		bool m_empty = true;
		bool m_supressIndent = false;
		size_t m_forceZeroConfidenceCount = 0;
		ScopeStyle m_scopeStyle = DefaultScopeStyle;

		void InitLine();

	public:
		DecompiledCTokenEmitter(BinaryViewRef view, BinaryNinja::DisassemblyTextRenderer* renderer = nullptr);

		template <typename ...Args>
		void Append(Args && ...args)
		{
			auto& line = GetCurrentLine();
			line.tokens.emplace_back(std::forward<Args>(args)...);
			if (m_forceZeroConfidenceCount > 0)
				line.tokens.back().confidence = 0;
		}
		...
		void SetCurrentExpr(const BinaryNinja::HighLevelILInstruction& expr);
		void Finalize();

		BinaryNinja::DisassemblyTextLine& GetCurrentLine();
		std::vector<BinaryNinja::InstructionTextToken>& GetCurrentTokens();

		const std::vector<BinaryNinja::DisassemblyTextLine>& GetLines() const { return m_lines; }
	};

	// class LinearViewOptionsWidget: public MenuHelper
	// {
	// public:
	// 	LinearViewOptionsWidget(DecompiledCView* parent);

	// protected:
	// 	virtual void showMenu();

	// private:
	// 	DecompiledCView* m_view;
	// };

	// class LinearViewStatusBarWidget: public StatusBarWidget
	// {
	// public:
	// 	LinearViewStatusBarWidget(DecompiledCView* parent);
...
public:
	explicit DecompiledCView(BinaryViewRef data, ViewFrame* view);
	virtual ~DecompiledCView();

	std::vector<BinaryNinja::LinearDisassemblyLine> generateLines();
	virtual void updateLines() override;

	void AddVarTextToken(const BinaryNinja::Variable& var, DecompiledCTokenEmitter& tokens,
		const BinaryNinja::HighLevelILInstruction& instr, size_t size);

	void GetExprText(const BinaryNinja::HighLevelILInstruction& instr, DecompiledCTokenEmitter& tokens,
		BinaryNinja::DisassemblySettings* settings, OperatorPrecedence precedence = TopLevelOperatorPrecedence);
...
class DecompiledCViewType: public ViewType
{
	static DecompiledCViewType* m_instance;
