...
void Md1romView::ParseDebugInfo()
{
	...
	// TODO: refactor the LZMA decompression code to a function
	lzma_stream strm = LZMA_STREAM_INIT;
	auto ret = lzma_auto_decoder(&strm, UINT64_MAX, 0);
	if (ret != LZMA_OK)
		return;
	...
	bool ok = true;
	strm.next_in = (const uint8_t*)data.GetData();
	strm.avail_in = data.GetLength();
	...
	while (true)
	{
		...
		{
			size_t size = sizeof(buffer) - strm.avail_out;
			decompressed.Append(buffer, size);
		}
		...
		if (ret != LZMA_OK)
		{
			...
			{
				m_logger->LogWarn("lzma error: %d", ret);
				ok = false;
				break;
			}
		}
