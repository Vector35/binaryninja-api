...
class SharedCacheBuilder
{
	...
	std::set<std::filesystem::path> m_processedFiles;
...
public:
	...
	std::set<std::filesystem::path> GetProcessedFiles() { return m_processedFiles; };
	...
	bool AddFile(
		const std::filesystem::path& filePath, const std::string& fileName, CacheEntryType cacheType = CacheEntryType::Secondary);

	// Process a directory on the file system.
	size_t AddDirectory(const std::filesystem::path& directoryPath);
